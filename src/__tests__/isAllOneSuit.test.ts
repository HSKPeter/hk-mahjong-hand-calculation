...
test("Classify a valid AllOneSuit Hand", () => {
    ...
    const tile5 = new Tile({suit: "dot", value: 8});
    ...
    expect(HandTypeFinder.isAllOneSuit(winningHand)).toBe(true);
});

test("Classify a Hand which is not AllOneSuit", () => {
    ...
    const winningHand = new WinningHand([meld1, meld2, meld3, meld4, meld5]);
    expect(HandTypeFinder.isAllOneSuit(winningHand)).toBe(false);
});