...
class Hand {
            ...
            if (inputConfig.tiles && inputConfig.tiles.length > 0){
                inputConfig.melds.forEach(meld => {
                    ...
                    if(meld.getMeldType() === MeldType.THIRTEEN_ORPHANS){
                        throw new Error("There should be no tiles remain unorganized if there the Meld is in the type of THIRTEEN_ORPHANS.");
                    }
                })
            }
    ...
    public isIdentical(handToBeCompared: Hand) {
        ...
        for (const tile of this.unorganizedTiles) {
            if (countTilesOccurrences(this.unorganizedTiles, tile) !== countTilesOccurrences(handToBeCompared.getUnorganizedTiles(), tile)) {
                return false;
            }
        }
        return true;
